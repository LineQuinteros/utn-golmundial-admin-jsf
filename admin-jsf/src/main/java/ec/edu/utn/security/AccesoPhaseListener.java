...
public class AccesoPhaseListener implements PhaseListener {
    ...
    public void afterPhase(PhaseEvent event) {
        ...
        if (!requiereAdmin && !requiereCliente) {
            return;
        }

        Application app = context.getApplication();
        LoginBean loginBean = app.evaluateExpressionGet(
            context,
            "#{loginBean}",
            LoginBean.class
        );
        ...
        try {
            ...
            if (requiereAdmin && !loginBean.isAdministrador()) {
                if (loginBean.isCliente()) {
                    redirigir(context, "/cliente/inicio.xhtml");
                } else {
                    redirigir(context, "/login.xhtml");
                }
                return;
            ...
            if (requiereCliente && !loginBean.isCliente()) {
                if (loginBean.isAdministrador()) {
                    redirigir(context, "/admin/inicio.xhtml");
                } else {
                    redirigir(context, "/login.xhtml");
                }
                return;
